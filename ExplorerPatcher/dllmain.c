...
{
    LAUNCHER_GROUP_VIEW_LIST = 0,
    LAUNCHER_GROUP_VIEW_LARGE_ICONS = 1,
    LAUNCHER_GROUP_VIEW_MEDIUM_ICONS = 2,
    LAUNCHER_GROUP_VIEW_SMALL_ICONS = 3
} LauncherGroupsViewMode;
...
typedef struct _LauncherGroup
{
    ...
    HICON hIconLarge;
    BOOL bMenuActive;
    BOOL bShowSettingsAfterMenu;
    BOOL bShowAppsAfterMenu;
    BOOL bAppsVisible;
    BOOL bKeepVisibleForModal;
    BOOL bDragActive;
...
LauncherGroup* g_launcherGroupsLastHoverGroup = NULL;
...
void LauncherGroups_FormatItemValue(LauncherGroupItem* item, WCHAR* value, size_t cchValue)
{
    if (!value || !cchValue)
    {
        return;
    ...
    swprintf_s(value, cchValue, L"%s|%s|%s", item->szName, item->szPath, item->szArgs);
}

BOOL LauncherGroups_SaveItemsToRegistry(LauncherGroup* group)
{
    HKEY hRoot = NULL;
    HKEY hGroupKey = NULL;
    BOOL ok = FALSE;
    ...
    if (RegOpenKeyExW(HKEY_CURRENT_USER, EP_LAUNCHER_GROUPS_REGPATH, 0, KEY_WRITE, &hRoot) == ERROR_SUCCESS)
    {
        if (RegOpenKeyExW(hRoot, group->szKeyName, 0, KEY_WRITE, &hGroupKey) == ERROR_SUCCESS)
        {
            ok = TRUE;
            ...
            for (DWORD i = 0; i < group->cItems; i++)
            {
                ...
                swprintf_s(valueName, ARRAYSIZE(valueName), L"Item%lu", i);
                LauncherGroups_FormatItemValue(&group->items[i], itemValue, ARRAYSIZE(itemValue));
                if (RegSetValueExW(hGroupKey, valueName, 0, REG_SZ, (const BYTE*)itemValue, ((DWORD)wcslen(itemValue) + 1) * sizeof(WCHAR)) != ERROR_SUCCESS)
                {
                    ok = FALSE;
                    break;
                }
    ...
    EPDebugLogWrite(L"launcher-group save items group=\"%s\" count=%lu ok=%d", group->szName, group->cItems, ok);
    return ok;
}

void LauncherGroups_RefreshGroupIcons(LauncherGroup* group)
{
    if (!group)
    {
        return;
...
void LauncherGroups_RefreshGroupAfterItemsChanged(LauncherGroup* group)
{
    if (!group)
    {
        return;
    ...
    LauncherGroups_RefreshGroupIcons(group);
    if (group->hWnd)
    {
...
void LauncherGroups_HideSystemPreviewWindows()
{
    EnumWindows(LauncherGroups_HideSystemPreviewEnumProc, 0);
}

HBITMAP LauncherGroups_CreateTransparentBitmap(int cx, int cy)
{
    BITMAPV5HEADER bi;
    void* pvBits = NULL;
    HDC hdc = GetDC(NULL);
    ...
    hBitmap = CreateDIBSection(hdc, (BITMAPINFO*)&bi, DIB_RGB_COLORS, &pvBits, NULL, 0);
    if (pvBits)
    {
        ZeroMemory(pvBits, cx * cy * 4);
    }
    if (hdc)
    {
        ReleaseDC(NULL, hdc);
...
void LauncherGroups_DisableSystemPreview(HWND hWnd)
{
    BOOL enabled = TRUE;
    ...
    DwmSetWindowAttribute(hWnd, DWMWA_DISALLOW_PEEK, &enabled, sizeof(enabled));
...
void LauncherGroups_UpdateWindowSize(LauncherGroup* group)
{
    RECT rc;
    SIZE size;
    ...
    if (GetWindowRect(group->hWnd, &rc))
    {
        SetWindowPos(group->hWnd, NULL, rc.left, rc.top, size.cx, size.cy, SWP_NOZORDER | SWP_NOACTIVATE);
        LauncherGroups_ResizeAppsListView(group);
    }
}

BOOL LauncherGroups_MoveItem(LauncherGroup* group, int from, int to)
{
    LauncherGroupItem item;
    ...
    item = group->items[from];
    if (from < to)
    {
        memmove(&group->items[from], &group->items[from + 1], (to - from) * sizeof(LauncherGroupItem));
    }
    else
    {
        memmove(&group->items[to + 1], &group->items[to], (from - to) * sizeof(LauncherGroupItem));
    }
    group->items[to] = item;

    if (LauncherGroups_SaveItemsToRegistry(group))
    {
        LauncherGroups_RefreshGroupAfterItemsChanged(group);
        if (group->hListView && IsWindow(group->hListView))
        {
            ListView_SetItemState(group->hListView, to, LVIS_SELECTED | LVIS_FOCUSED, LVIS_SELECTED | LVIS_FOCUSED);
        }
...
BOOL LauncherGroups_RemoveItem(LauncherGroup* group, int itemIndex)
{
    ...
    if (group->items[itemIndex].hIconSmall)
    {
        DestroyIcon(group->items[itemIndex].hIconSmall);
    }
    if (group->items[itemIndex].hIconLarge)
    {
        DestroyIcon(group->items[itemIndex].hIconLarge);
    }
    if (itemIndex < (int)group->cItems - 1)
    {
        memmove(&group->items[itemIndex], &group->items[itemIndex + 1], (group->cItems - itemIndex - 1) * sizeof(LauncherGroupItem));
    }
    group->cItems--;
    ZeroMemory(&group->items[group->cItems], sizeof(LauncherGroupItem));

    if (LauncherGroups_SaveItemsToRegistry(group))
    {
        LauncherGroups_RefreshGroupAfterItemsChanged(group);
        EPDebugLogWrite(L"launcher-group remove item group=\"%s\" index=%d", group->szName, itemIndex);
        return TRUE;
    }
    return FALSE;
}
...
LauncherGroup* LauncherGroups_FindByName(LPCWSTR name)
{
    if (!name || !name[0])
    {
        return NULL;
    ...
    for (LauncherGroup* group = g_launcherGroups; group; group = group->next)
    {
        size_t cchName = wcslen(group->szName);
        if (cchName
            && !_wcsnicmp(name, group->szName, cchName)
            && (!name[cchName] || iswspace(name[cchName]) || wcschr(L"-([", name[cchName])))
        {
            return group;
        }
    }
...
BOOL LauncherGroups_IsPointOnPopupMenu(POINT pt)
{
    HWND hWnd = WindowFromPoint(pt);
    while (hWnd)
    {
...
BOOL LauncherGroups_IsPointOnAppsWindow(LauncherGroup* group, POINT pt)
{
    HWND hWnd;
    RECT rc;
    ...
    if (GetWindowRect(group->hWnd, &rc) && PtInRect(&rc, pt))
    {
        return TRUE;
    }
    ...
    while (hWnd)
    {
        if (hWnd == group->hWnd || hWnd == group->hListView)
        {
            return TRUE;
        }
        hWnd = GetParent(hWnd);
...
LauncherGroup* LauncherGroups_FindGroupAtPoint(POINT pt)
{
    ...
    IUIAutomationElement* pCurrentElement = NULL;
    ...
    if (SUCCEEDED(hr) && pIUIAutomation2)
    {
        pIUIAutomation2->lpVtbl->get_ControlViewWalker(pIUIAutomation2, &pControlWalker);
    }

    pCurrentElement = pIUIAutomationElement;
    pIUIAutomationElement = NULL;
    for (int i = 0; pCurrentElement && i < 5 && !group; i++)
    {
        elemName = NULL;
        hr = pCurrentElement->lpVtbl->get_CurrentName(pCurrentElement, &elemName);
        if (SUCCEEDED(hr) && elemName)
        {
            group = LauncherGroups_FindByName(elemName);
        }
        if (elemName)
        {
        ...
        if (!group && pControlWalker)
        {
            IUIAutomationElement* pParentElement = NULL;
            pControlWalker->lpVtbl->GetParentElement(pControlWalker, pCurrentElement, &pParentElement);
            pCurrentElement->lpVtbl->Release(pCurrentElement);
            pCurrentElement = pParentElement;
        }
        else if (!group)
        {
            break;
...
BOOL LauncherGroups_ShouldKeepAppsWindowOpen(LauncherGroup* group)
{
    ...
    GetCursorPos(&pt);
    if (group->bMenuActive || group->bKeepVisibleForModal || LauncherGroups_IsPointOnPopupMenu(pt) || LauncherGroups_IsPointOnAppsWindow(group, pt))
    {
        return TRUE;
    }

    groupAtPoint = LauncherGroups_FindGroupAtPoint(pt);
    if (groupAtPoint == group)
    {
        g_launcherGroupsLastHoverGroup = group;
        g_launcherGroupsLastHoverTick = GetTickCount();
        LauncherGroups_HideSystemPreviewWindows();
        return TRUE;
    }

    if (g_launcherGroupsLastHoverGroup == group
...
BOOL LauncherGroups_OnTaskbarMouseMove(POINT pt)
{
    ...
    {
        BOOL cachedGroupHit = g_launcherGroupsLastHoverGroup
        ...
        return cachedGroupHit;
    }

    g_launcherGroupsLastMouseCheckTick = tick;
    group = LauncherGroups_FindGroupAtPoint(pt);
    ...
    g_launcherGroupsLastHoverGroup = group;
    g_launcherGroupsLastHoverTick = tick;
    LauncherGroups_HideSystemPreviewWindows();
...
void LauncherGroups_PostViewModeChanged()
{
    if (g_launcherGroupsThreadId)
    {
...
DWORD LauncherGroups_ClampViewMode(DWORD viewMode)
{
    if (viewMode > LAUNCHER_GROUP_VIEW_SMALL_ICONS)
    {
        return LAUNCHER_GROUP_VIEW_LIST;
    }
    return viewMode;
}

DWORD LauncherGroups_ReadViewModeFromRegistry()
{
    HKEY hKey = NULL;
    DWORD viewMode = LAUNCHER_GROUP_VIEW_LIST;
    DWORD cb = sizeof(viewMode);
    DWORD type = REG_DWORD;

    if (RegOpenKeyExW(HKEY_CURRENT_USER, TEXT(REGPATH), 0, KEY_READ, &hKey) == ERROR_SUCCESS)
    {
        RegQueryValueExW(hKey, EP_LAUNCHER_GROUP_VIEWMODE_VALUE, NULL, &type, (BYTE*)&viewMode, &cb);
        RegCloseKey(hKey);
    }
    return LauncherGroups_ClampViewMode(viewMode);
}

void LauncherGroups_SetViewMode(DWORD viewMode)
{
    viewMode = LauncherGroups_ClampViewMode(viewMode);
    if (g_launcherGroupsViewMode != viewMode)
    {
        g_launcherGroupsViewMode = viewMode;
        LauncherGroups_PostViewModeChanged();
    }
}
...
int LauncherGroups_GetAppsIconSize(DWORD viewMode)
{
    ...
    case LAUNCHER_GROUP_VIEW_LARGE_ICONS:
        return max(48, GetSystemMetrics(SM_CXICON));
...
DWORD LauncherGroups_GetAppsListViewStyle()
{
    ...
    case LAUNCHER_GROUP_VIEW_LIST:
    default:
        return LVS_REPORT | LVS_NOCOLUMNHEADER;
    }
}
...
void LauncherGroups_DestroyAppsImageLists(LauncherGroup* group)
{
    ...
    if (group->hListView && IsWindow(group->hListView))
    {
        SendMessageW(group->hListView, LVM_SETIMAGELIST, LVSIL_SMALL, 0);
        SendMessageW(group->hListView, LVM_SETIMAGELIST, LVSIL_NORMAL, 0);
    }
    if (group->hImageListSmall)
    {
        ImageList_Destroy(group->hImageListSmall);
        group->hImageListSmall = NULL;
    }
    if (group->hImageListMedium)
    {
        ImageList_Destroy(group->hImageListMedium);
        group->hImageListMedium = NULL;
    }
    if (group->hImageListLarge)
    {
        ImageList_Destroy(group->hImageListLarge);
        group->hImageListLarge = NULL;
    }
}

void LauncherGroups_CreateAppsImageLists(LauncherGroup* group)
{
    int smallSize = LauncherGroups_GetAppsIconSize(LAUNCHER_GROUP_VIEW_SMALL_ICONS);
    int mediumSize = LauncherGroups_GetAppsIconSize(LAUNCHER_GROUP_VIEW_MEDIUM_ICONS);
    int largeSize = LauncherGroups_GetAppsIconSize(LAUNCHER_GROUP_VIEW_LARGE_ICONS);
    int initialCount = group && group->cItems ? group->cItems : 1;
    ...
    group->hImageListSmall = ImageList_Create(smallSize, smallSize, ILC_COLOR32 | ILC_MASK, initialCount, 1);
    group->hImageListMedium = ImageList_Create(mediumSize, mediumSize, ILC_COLOR32 | ILC_MASK, initialCount, 1);
    group->hImageListLarge = ImageList_Create(largeSize, largeSize, ILC_COLOR32 | ILC_MASK, initialCount, 1);

    for (DWORD i = 0; i < group->cItems; i++)
    {
        LauncherGroupItem* item = &group->items[i];
        HICON hSmallIcon = item->hIconSmall ? item->hIconSmall : item->hIconLarge;
        HICON hLargeIcon = item->hIconLarge ? item->hIconLarge : item->hIconSmall;

        if (group->hImageListSmall && hSmallIcon)
        {
            ImageList_AddIcon(group->hImageListSmall, hSmallIcon);
        }
        if (group->hImageListMedium && hLargeIcon)
        {
            ImageList_AddIcon(group->hImageListMedium, hLargeIcon);
        }
        if (group->hImageListLarge && hLargeIcon)
        {
            ImageList_AddIcon(group->hImageListLarge, hLargeIcon);
        }
    }
}
...
BOOL LauncherGroups_EnsureAppsListView(HWND hWnd, LauncherGroup* group)
{
    ...
    group->hListView = CreateWindowExW(
        ...
        0,
        hWnd,
        (HMENU)EP_LAUNCHER_GROUP_LISTVIEW_ID,
...
void LauncherGroups_ResizeAppsListView(LauncherGroup* group)
{
    ...
    if (!group || !group->hWnd || !group->hListView || !IsWindow(group->hListView))
    {
        return;
    }

    GetClientRect(group->hWnd, &rc);
    MoveWindow(group->hListView, 8, 8, max(0, rc.right - 16), max(0, rc.bottom - 16), TRUE);
    if (g_launcherGroupsViewMode == LAUNCHER_GROUP_VIEW_LIST)
    {
        ListView_SetColumnWidth(group->hListView, 0, max(0, rc.right - 28));
    }
}
...
void LauncherGroups_UpdateAppsWindow(LauncherGroup* group)
{
    ...
    if (g_launcherGroupsViewMode == LAUNCHER_GROUP_VIEW_LIST)
    {
        ...
        column.cx = 280;
        ListView_InsertColumn(group->hListView, 0, &column);
    }

    SendMessageW(group->hListView, LVM_SETIMAGELIST, LVSIL_SMALL, (LPARAM)group->hImageListSmall);
    if (g_launcherGroupsViewMode == LAUNCHER_GROUP_VIEW_LARGE_ICONS)
    {
        SendMessageW(group->hListView, LVM_SETIMAGELIST, LVSIL_NORMAL, (LPARAM)group->hImageListLarge);
    }
    else if (g_launcherGroupsViewMode == LAUNCHER_GROUP_VIEW_MEDIUM_ICONS)
    {
        SendMessageW(group->hListView, LVM_SETIMAGELIST, LVSIL_NORMAL, (LPARAM)group->hImageListMedium);
    }
    else
    {
        SendMessageW(group->hListView, LVM_SETIMAGELIST, LVSIL_NORMAL, (LPARAM)group->hImageListSmall);
    }
...
void LauncherGroups_PositionAppsWindow(LauncherGroup* group)
{
    ...
    x = max(mi.rcWork.left, min(x, mi.rcWork.right - size.cx));
    y = max(mi.rcWork.top, min(y, mi.rcWork.bottom - size.cy));
...
void LauncherGroups_HideAppsWindow(LauncherGroup* group)
{
    if (!group || !group->hWnd)
    {
        return;
    }
    if (group->bKeepVisibleForModal)
    {
        return;
    }

    group->bAppsVisible = FALSE;
    KillTimer(group->hWnd, EP_LAUNCHER_GROUP_HOVER_TIMER);
    SIZE size = LauncherGroups_GetAppsWindowSize(group);
    SetWindowPos(group->hWnd, NULL, -32000, -32000, size.cx, size.cy, SWP_NOZORDER | SWP_NOACTIVATE | SWP_SHOWWINDOW);
    ShowWindow(group->hWnd, SW_SHOWNOACTIVATE);
}

void LauncherGroups_HideOtherAppsWindows(LauncherGroup* keepGroup)
{
    for (LauncherGroup* group = g_launcherGroups; group; group = group->next)
    {
        if (group != keepGroup && group->bAppsVisible)
        {
...
void LauncherGroups_ShowAppsWindow(HWND hWnd, LauncherGroup* group)
{
    if (!group || !hWnd)
    {
        return;
    ...
    if (group->bMenuActive)
    {
        group->bShowAppsAfterMenu = TRUE;
        EndMenu();
        return;
    }
    if (group->bAppsVisible)
    {
        LauncherGroups_PositionAppsWindow(group);
        SetTimer(hWnd, EP_LAUNCHER_GROUP_HOVER_TIMER, 120, NULL);
        return;
    }

    LauncherGroups_HideOtherAppsWindows(group);
    LauncherGroups_HideSystemPreviewWindows();
    LauncherGroups_UpdateAppsWindow(group);
    LauncherGroups_PositionAppsWindow(group);
    group->bAppsVisible = TRUE;
    SetTimer(hWnd, EP_LAUNCHER_GROUP_HOVER_TIMER, 120, NULL);
    EPDebugLogWrite(L"launcher-group apps window show hwnd=%p group=\"%s\" items=%lu mode=%lu", hWnd, group->szName, group->cItems, g_launcherGroupsViewMode);
}

void LauncherGroups_RefreshOpenAppsWindows()
{
    for (LauncherGroup* group = g_launcherGroups; group; group = group->next)
    {
        LauncherGroups_UpdateAppsWindow(group);
...
BOOL LauncherGroups_RenameRegistryGroup(LauncherGroup* group)
{
    ...
    HRESULT hrInput = InputBox(FALSE, group->hWnd, L"\u8F93\u5165\u5206\u7EC4\u540D\u79F0", L"\u4FEE\u6539\u5206\u7EC4\u540D", group->szName, newName, ARRAYSIZE(newName), &cancelled);
    group->bKeepVisibleForModal = FALSE;
    if (FAILED(hrInput) || cancelled || !newName[0])
    {
        return FALSE;
    }
...
void LauncherGroups_ShowSettingsMenu(HWND hWnd, LauncherGroup* group)
{
    ...
    EPDebugLogWrite(L"launcher-group menu enter hwnd=%p group=\"%s\" items=%lu mode=settings", hWnd, group->szName, group->cItems);
    group->bMenuActive = TRUE;
    group->dwMenuMode = EP_LAUNCHER_GROUP_MENU_MODE_SETTINGS;
    group->bShowSettingsAfterMenu = FALSE;
    group->bShowAppsAfterMenu = FALSE;
    ...
    if (group->cItems)
    {
        ...
        if (hRemoveMenu)
        {
            ...
            {
                WCHAR itemText[180];
                swprintf_s(itemText, ARRAYSIZE(itemText), L"%lu. %s", i + 1, group->items[i].szName);
                AppendMenuW(hRemoveMenu, MF_STRING, EP_LAUNCHER_GROUP_MENU_REMOVE_FIRST + i, itemText);
            }
            AppendMenuW(hMenu, MF_POPUP | MF_STRING, (UINT_PTR)hRemoveMenu, L"\u79FB\u9664\u5E94\u7528");
            AppendMenuW(hMenu, MF_SEPARATOR, 0, NULL);
        }
    }
    ...
    BOOL bShowSettingsAfterMenu = group->bShowSettingsAfterMenu;
    BOOL bShowAppsAfterMenu = group->bShowAppsAfterMenu;
    group->bShowSettingsAfterMenu = FALSE;
    group->bShowAppsAfterMenu = FALSE;
    group->bMenuActive = FALSE;
    group->dwMenuMode = EP_LAUNCHER_GROUP_MENU_MODE_NONE;
    if (cmd == EP_LAUNCHER_GROUP_MENU_ADD_FOREGROUND
        || cmd == EP_LAUNCHER_GROUP_MENU_RENAME
        || cmd == EP_LAUNCHER_GROUP_MENU_DISSOLVE)
    {
        bShowSettingsAfterMenu = FALSE;
        bShowAppsAfterMenu = FALSE;
    }
    if (cmd == EP_LAUNCHER_GROUP_MENU_ADD_FOREGROUND)
    {
    ...
    {
        LauncherGroups_RemoveItem(group, cmd - EP_LAUNCHER_GROUP_MENU_REMOVE_FIRST);
    }

    if (!group->bAppsVisible)
    {
        SIZE size = LauncherGroups_GetAppsWindowSize(group);
        SetWindowPos(hWnd, NULL, -32000, -32000, size.cx, size.cy, SWP_NOZORDER | SWP_NOACTIVATE | SWP_SHOWWINDOW);
        ShowWindow(hWnd, SW_SHOWNOACTIVATE);
    }
    if (bShowSettingsAfterMenu)
    {
        PostMessageW(hWnd, EP_LAUNCHER_GROUP_SHOW_MENU_MSG, 0, 0);
    }
    else if (bShowAppsAfterMenu)
    {
        PostMessageW(hWnd, EP_LAUNCHER_GROUP_SHOW_APPS_MSG, 0, 0);
    }
...
int LauncherGroups_HitTestListViewItem(LauncherGroup* group)
{
    ...
    hitTest.pt.x = GET_X_LPARAM(messagePos);
    hitTest.pt.y = GET_Y_LPARAM(messagePos);
    ScreenToClient(group->hListView, &hitTest.pt);
...
int LauncherGroups_HitTestListViewItemAtPoint(LauncherGroup* group, POINT ptScreen)
{
    LVHITTESTINFO hitTest;
...
LRESULT CALLBACK LauncherGroups_WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        LauncherGroups_DisableSystemPreview(hWnd);
        if (group)
        {
            group->hWnd = hWnd;
    ...
    {
        HBITMAP hBitmap = LauncherGroups_CreateTransparentBitmap(HIWORD(lParam), LOWORD(lParam));
        if (hBitmap)
        {
    ...
    {
        HBITMAP hBitmap = LauncherGroups_CreateTransparentBitmap(1, 1);
        if (hBitmap)
        {
            POINT ptZero = { 0, 0 };
            DwmSetIconicLivePreviewBitmap(hWnd, hBitmap, &ptZero, 0);
            DeleteObject(hBitmap);
        }
        return 0;
    }
    case EP_LAUNCHER_GROUP_SHOW_MENU_MSG:
        if (group && group->bMenuActive)
        {
            group->bShowSettingsAfterMenu = TRUE;
            EndMenu();
            return 0;
        }
        LauncherGroups_ShowSettingsMenu(hWnd, group);
        return 0;
    case EP_LAUNCHER_GROUP_SHOW_APPS_MSG:
        LauncherGroups_ShowAppsWindow(hWnd, group);
        return 0;
    case EP_LAUNCHER_GROUP_VIEWMODE_MSG:
        if (group)
        {
            LauncherGroups_UpdateAppsWindow(group);
            if (group->bAppsVisible)
        ...
        return 0;
    case WM_TIMER:
        if (wParam == EP_LAUNCHER_GROUP_HOVER_TIMER)
        {
            if (!group || !group->bAppsVisible)
            {
                KillTimer(hWnd, EP_LAUNCHER_GROUP_HOVER_TIMER);
                return 0;
            }
            if (!LauncherGroups_ShouldKeepAppsWindowOpen(group))
            {
                LauncherGroups_HideAppsWindow(group);
            }
            return 0;
    ...
    case WM_SYSCOMMAND:
        ...
        break;
    case WM_ACTIVATE:
        if (LOWORD(wParam) != WA_INACTIVE && group && !group->bMenuActive && !group->bKeepVisibleForModal)
        {
            PostMessageW(hWnd, EP_LAUNCHER_GROUP_SHOW_APPS_MSG, 0, 0);
            return 0;
        }
    ...
    case WM_SIZE:
        ...
        break;
    case WM_NOTIFY:
        if (group && ((LPNMHDR)lParam)->hwndFrom == group->hListView)
        {
...
DWORD LauncherGroupsThread(DWORD unused)
{
    ...
    while (GetMessageW(&msg, NULL, 0, 0) > 0)
    {
        ...
        if (msg.message == EP_LAUNCHER_GROUP_VIEWMODE_MSG)
        {
            ...
            LauncherGroups_RefreshOpenAppsWindows();
            continue;
        }
...
#if WITH_MAIN_PATCHER
        dwSize = sizeof(DWORD);
        dwTemp = LAUNCHER_GROUP_VIEW_LIST;
        RegQueryValueExW(
            hKey,
            EP_LAUNCHER_GROUP_VIEWMODE_VALUE,
            0,
            NULL,
            &dwTemp,
            &dwSize
        );
        LauncherGroups_SetViewMode(dwTemp);
#endif
        dwSize = sizeof(DWORD);
        dwTemp = 0;
        RegQueryValueExW(
...
#if WITH_MAIN_PATCHER
HRESULT explorer_DwmRegisterThumbnailHook(HWND hwndDestination, HWND hwndSource, PHTHUMBNAIL phThumbnailId)
{
    if (LauncherGroups_IsLauncherGroupWindow(hwndSource))
    {
        if (phThumbnailId)
        {
            *phThumbnailId = NULL;
        ...
        LauncherGroups_HideSystemPreviewWindows();
        EPDebugLogWrite(L"launcher-group block taskbar thumbnail destination=%p source=%p", hwndDestination, hwndSource);
        return E_INVALIDARG;
    }

    return DwmRegisterThumbnail(hwndDestination, hwndSource, phThumbnailId);
}
...
DWORD Inject(BOOL bIsExplorer)
{
    ...
    VnPatchIAT(hExplorer, "dwmapi.dll", "DwmRegisterThumbnail", explorer_DwmRegisterThumbnailHook);
    ...
    if (hMyTaskbar)
    {
        ...
        VnPatchIAT(hMyTaskbar, "dwmapi.dll", "DwmRegisterThumbnail", explorer_DwmRegisterThumbnailHook);
